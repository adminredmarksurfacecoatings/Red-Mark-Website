import type { FinishCollectionConfig, FinishSpecIcon } from '@/lib/finishCollection/types'

type FinishCollectionSpecBarProps = {
  config: FinishCollectionConfig
}

function SpecIcon({ type }: { type: FinishSpecIcon }) {
...
export default function FinishCollectionSpecBar({ config }: FinishCollectionSpecBarProps) {
  return (
    <section className="stone-finish-spec page-section" aria-label={config.specAriaLabel}>
      <div className="stone-finish-spec__container container">
        <div className="stone-finish-spec__bar">
          {config.specs.map((spec, index) => (
            <article
              key={spec.label}
              className={`stone-finish-spec__item${index < config.specs.length - 1 ? ' stone-finish-spec__item--divider' : ''}`}
            >
