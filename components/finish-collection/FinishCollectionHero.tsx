...
import FinishCollectionBreadcrumbs from '@/components/finish-collection/FinishCollectionBreadcrumbs'
import type { FinishCollectionConfig, FinishFeatureIcon } from '@/lib/finishCollection/types'

type FinishCollectionHeroProps = {
  config: FinishCollectionConfig
}

function HeroFeatureIcon({ type }: { type: FinishFeatureIcon }) {
...
export default function FinishCollectionHero({ config }: FinishCollectionHeroProps) {
  return (
    <section className="stone-finish-hero page-section page-section--first">
      <div className="stone-finish-hero__container container">
        <FinishCollectionBreadcrumbs config={config} />

        <div className="stone-finish-hero__grid">
          <div className="stone-finish-hero__content">
            <p className="stone-finish-hero__eyebrow">{config.eyebrow}</p>
            <h1 className="stone-finish-hero__heading">{config.collectionTitle}</h1>
            <p className="stone-finish-hero__lead">{config.heroLead}</p>
            ...
            <div className="stone-finish-hero__actions">
              ...
              <Link href={config.requestSampleHref} className="stone-finish-hero__sample">
                Request Sample
              </Link>
            </div>
          </div>
          ...
          <div className="stone-finish-hero__visual">
            <div className="stone-finish-hero__image-wrap">
              <FinishCollectionAssetImage
                primarySrc={config.heroImage}
                fallbackSrc={config.heroImageFallback}
